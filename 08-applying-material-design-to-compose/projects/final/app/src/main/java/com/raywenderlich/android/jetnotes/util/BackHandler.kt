...
/**
 * This [Composable] can be used with a [BackPressedDispatcher] to intercept a back press (if
 * [enabled]).
 */
@Composable
fun BackPressHandler(onBackPressed: () -> Unit, enabled: Boolean = true) {
    val dispatcher = BackPressedDispatcher.current.onBackPressedDispatcher
